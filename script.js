...
function refreshCompiled() {
  window.compiled_ix = [];
  Array.from(ix_list.childNodes)
    .filter((a) => a.tagName == "LI")
    .forEach((i) => {
      let details = {
        name: i.childNodes.item(0).textContent,
        args: {},
      };
      let args = Array.from(i.childNodes.item(1).childNodes).filter(
        (a) => a.tagName == "LI"
      );
      args.forEach((a) => {
...
function addIx(i, args) {
  ...
  Object.keys(ix_record.args).forEach((k, i) => {
    ...
    if (ix_record.args[k].type === "number" && arginpt.value == "")
      arginpt.value = ix_record.args[k].default;
